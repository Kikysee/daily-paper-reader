...
tags: ["query:q8"]
score: 7.0
evidence: 利用遥感监测全球土地覆盖动态
tldr: 现有全球陆海覆盖监测在时效性和耦合分析上存在不足。本研究开发了一个融合多源数据的综合监测框架，生成了5天分辨率、具备近实时更新能力的全球陆地和海冰覆盖时间序列（2018-2025）。研究揭示了陆海覆盖的时空动态特征，并结合地球系统安全阈值发出预警，为全球气候适应和可持续决策提供了重要的科学支撑。
source: arxiv
...
motivation: 现有全球陆海覆盖数据集在时间分辨率、时效性、陆海耦合分析以及地球系统安全阈值视角方面存在明显局限。
method: 融合多源遥感和再分析数据，构建综合监测框架，生成5天分辨率且具备近实时更新能力的全球陆地与海冰覆盖时间序列。
result: 揭示了植被和海冰的显著季节性与区域性动态，并发现全球森林覆盖率正逼近安全下限，北极夏季海冰覆盖率偶尔跌破临界阈值。
