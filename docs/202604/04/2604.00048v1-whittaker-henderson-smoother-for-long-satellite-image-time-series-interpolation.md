...
## 摘要
Whittaker平滑器是预处理卫星图像时间序列（SITS）的一种广泛采用的解决方案。然而，它仍存在两个关键局限性：必须为每个像素单独调整平滑参数，且标准公式假设噪声是同方差的，从而在时间维度上施加均匀的平滑。本文通过将Whittaker平滑器构建为可微神经层来解决这两个局限性，其中平滑参数由神经网络推断。该框架进一步扩展以通过时变正则化处理异方差噪声，允许平滑程度沿时间序列局部自适应。为了实现大规模处理，提出了一种稀疏、内存高效且完全可微的实现，通过Cholesky分解利用底层线性系统的对称带状结构。在GPU上的基准测试表明，该实现在速度和内存消耗方面均大幅优于标准的密集线性求解器。该方法在2016年至2024年间获取的法国本土SITS上进行了验证。结果证实了大规模异方差Whittaker平滑的可行性，尽管与同方差基线的重建差异仍然有限，这表明用于平滑参数估计的Transformer架构可能缺乏捕捉单日云污染等突发噪声变化所需的时间敏锐度。
